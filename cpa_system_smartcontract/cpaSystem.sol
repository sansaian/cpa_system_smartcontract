...
contract Registrator  {
     ...
     */
     ...
     mapping (address => address) private dealsStorage;
     mapping (address => uint) private usersStorage;
     address public cashierContract;

    /*
     *  Events
     */
     event LogGetDealAddress(address _advertiser, address _smartDeal);
     ...
     */
     function Registrator(){
        owner = msg.sender;
        //create Cashier
        cashierContract = new Cashier(owner);
    }
    ...
    */
    function registrationUser(address _user,uint _role) isOwner returns (uint) {
        usersStorage[_user] = _role;
        LogStoreUser(_user,  usersStorage[_user]);
        return usersStorage[_user];
    }
    ...
    function getAddress(address _advertiser) constant returns (address) {
        LogGetDealAddress(_advertiser,  dealsStorage[_advertiser]);
        return dealsStorage[_advertiser];
    }
    ...
    */
    function createSmartDeal(address _advertiser,uint costClick)isOwner returns (address) {
        if(cashierContract != 0x0){
            dealsStorage[_advertiser] = new SmartDeal(cashierContract,_advertiser ,costClick);
        }
        LogResultCreateSmartDeal( _advertiser,dealsStorage[_advertiser]);
        return dealsStorage[_advertiser];
    }
...
contract Cashier {
     ...
     */
     ...
     function Cashier(address _owner){
